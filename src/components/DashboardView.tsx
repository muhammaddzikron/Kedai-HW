...
export default function DashboardView({ onOpenReceipt }: { onOpenReceipt: (order: any) => void }) {
  const { orders, products, auditLogs, currentUser, currentBranch, onlineOrders } = useApp();
  const [activeSubTab, setActiveSubTab] = useState<'ANALYTICS' | 'MARKETPLACE'>('ANALYTICS');
          ...
          <div className="flex bg-slate-200/50 p-1 rounded-xl">
            ...
            >
              ...
              {pendingApprovalsCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-rose-500 text-white text-[9px] h-4.5 min-w-4.5 px-1 rounded-full flex items-center justify-center font-bold animate-pulse shadow-sm">
            ...
            <div className="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm lg:col-span-2">
              ...
              {stats.todayOrdersList.length > 0 ? (
                ...
                <div className="h-40 flex flex-col items-center justify-center text-slate-400 text-center bg-slate-50/50 rounded-xl border border-dashed border-slate-200 mt-2">
                  ...
                  <p className="text-[10px]">Silakan buka menu POS Kasir untuk mencatat transaksi.</p>
                </div>
              )}
            </div>
